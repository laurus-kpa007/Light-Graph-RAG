...
```mermaid
graph LR
    A[DocxConverter] --> B[MarkdownParser]
    B --> C[Chunker]
...
### 1.2 DocxConverter (docx_converter.py)

**책임**: .docx 파일을 마크다운 형식으로 변환

**클래스 설계**:
```python
class DocxConverter:
    """
    .docx 파일을 마크다운으로 변환하는 클래스
    markitdown 라이브러리를 활용하여 표 구조를 유지하면서 변환
    """
    ...
    def convert_directory(self, dir_path: str,
        ...
        Args:
            dir_path: .docx 파일들이 있는 디렉토리 경로
            output_dir: 변환된 마크다운 저장 경로 (선택)

        Returns:
            List[Dict]: 변환 결과 메타데이터 리스트
        """
...
```mermaid
flowchart TD
    A[.docx 파일 입력] --> B{파일 존재?}
    B -->|No| C[에러 반환]
    B -->|Yes| D[MarkItDown 변환]
    D --> E[마크다운 텍스트 추출]
    E --> F{표 구조 존재?}
    F -->|Yes| G[표 포맷팅 정규화]
    F -->|No| H[헤더 구조 검증]
...
```python
{
    "original_path": "/path/to/file.docx",
    "filename": "사내규정_001.docx",
